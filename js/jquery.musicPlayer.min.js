...
(function($){$.musicPlayer=function(options){var defaults={autoplay:false,loop:false,position:"bottom"};var settings=$.extend({},defaults,options);var musicPlayerContainer=$("<div id='music-player-container'>");var playControls='<div id="play-controls">'+'<a id="play-prev" class="player-icon "></a>'+'<a id="play-pause" class="player-icon "></a>'+'<a id="play-next" class="player-icon "></a>'+'<a id="shuffle" class="player-icon" title="aleatorio"></a>'+"</div>";var sliderWrap='<div id="slider-wrapper">'+
'<span id="song-name">namesong </span>'+'<span id="song-time"><span id="current-time">0:00</span> / <span id="song-duration">00:00</span></span>'+'<div id="progress-slider"></div>'+'<img src="img/loading.gif" id="loading" style="display:none">'+"</div>";var playerOptions='<div id="player-options">'+'<div id="wrapper-volumen">'+'<div id="volumen-slider" style="position: absolute;top: -95px;margin-left: 7px;"></div>'+'<a id="player-volumen" class="player-icon"></a>'+"</div>"+"</div>";var player=musicPlayerContainer.html(playControls+
sliderWrap+playerOptions);if(settings.position==="bottom")player.appendTo("body");else{player.appendTo($(settings.position));$("#music-player-container").css({"height":"45px","padding":"6px","position":"relative"});$("#play-controls").css("margin-left","0%");$("#volumen-slider").css("top","-90px")}var arraySongs=[];var currentSong;var intervalProgress;var nextPrevPressed=false;var playing=false;var progresSlider=$("#progress-slider");var volumen=true;var shuffle=false;function getFirstAudioSrc(){$.each(options.sourceAudio,
function(index,val){if(index===0)currentSong=val.src;arraySongs.push(val.src)})}function createAudio(){soundManager.setup({url:"swf/",preferFlash:false,onready:function(){var mySound=soundManager.createSound({id:"aSound",url:currentSong,onload:function(){musicPlayer.setDurationSong(convertMS(this.duration));var that=this;musicPlayer.elements.loading.hide();initSliderSong(that);if(localStorage.volumen==0)soundManager.setVolume("aSound",0);else soundManager.setVolume("aSound",getCurrentVolumenStorage())},
onfinish:function(){musicPlayer.handlerNextSong("next")}});if(settings.autoplay||nextPrevPressed){progresSlider.hide();musicPlayer.elements.loading.show();mySound.play();musicPlayer.controls.playPause.addClass("playing")}},ontimeout:function(){}})}function convertMS(ms){var d,h,m,s;s=Math.floor(ms/1E3);m=Math.floor(s/60);s=s%60;h=Math.floor(m/60);m=m%60;d=Math.floor(h/24);h=h%24;if(s<10)s="0"+s;return m+":"+s}function initSliderSong(that){progresSlider.slider({range:"min",min:0,max:that.duration,
value:1,step:0.1,change:function(e,ui){if(e.eventPhase)soundManager.setPosition("aSound",ui.value)}}).show();setIntervalSong(that)}function setIntervalSong(that){intervalProgress=setInterval(function(){progresSlider.slider("option","value",that.position);musicPlayer.elements.currentTime.text(convertMS(that.position))},1E3)}function initVolumenSlider(){$("#volumen-slider").slider({orientation:"vertical",range:"min",min:0,max:100,value:getCurrentVolumenStorage(),step:10,change:function(event,ui){soundManager.setVolume("aSound",
ui.value);localStorage.currentVolumen=ui.value;localStorage.volumen=1;musicPlayer.controls.muteVolumen.removeClass("mute")}})}function resetAudio(){soundManager.destroySound("aSound");nextPrevPressed=true;clearInterval(intervalProgress);createAudio();musicPlayer.setNameSongPlayer(currentSong)}function supportStorage(){try{return"localStorage"in window&&window.localStorage!==null}catch(e){return false}}function setMute(){if(supportStorage())if(localStorage.volumen==0){musicPlayer.controls.muteVolumen.addClass("mute");
volumen=false}}function getCurrentVolumenStorage(){return localStorage.currentVolumen?localStorage.currentVolumen:100}function getRandomSong(){var len=arraySongs.length;var rand=Math.floor(Math.random()*len);var randomSong=arraySongs[rand];while(currentSong===randomSong){rand=Math.floor(Math.random()*len);randomSong=arraySongs[rand]}currentSong=arraySongs[rand];return arraySongs[rand]}getFirstAudioSrc();createAudio();initVolumenSlider();musicPlayer={controls:{playPause:$("#play-pause"),next:$("#play-next"),
prev:$("#play-prev"),muteVolumen:$("#player-volumen"),shuffle:$("#shuffle")},elements:{songName:$("#song-name"),songDuration:$("#song-duration"),currentTime:$("#current-time"),wrapVolumen:$("#wrapper-volumen"),volumenSlider:$("#volumen-slider"),loading:$("#loading")},playPause:function(){musicPlayer.controls.playPause.on("click",function(e){e.preventDefault();var that=$(this);if(!that.hasClass("playing")){that.addClass("playing");soundManager.play("aSound")}else{that.removeClass("playing");soundManager.pause("aSound")}})},
setNameSongPlayer:function(){musicPlayer.elements.songName.text(currentSong)},setDurationSong:function(duration){musicPlayer.elements.songDuration.text(duration)},setVolumenSlider:function(){musicPlayer.elements.wrapVolumen.hover(function(){musicPlayer.elements.volumenSlider.show()},function(){musicPlayer.elements.volumenSlider.hide()})},muteVolumen:function(){musicPlayer.controls.muteVolumen.on("click",function(e){e.preventDefault();var that=$(this);if(!that.hasClass("mute")){that.addClass("mute");
soundManager.setVolume("aSound",0);localStorage.volumen=0}else{that.removeClass("mute");soundManager.setVolume("aSound",100);localStorage.volumen=1}})},playNextSong:function(){musicPlayer.controls.next.on("click",function(e){e.preventDefault();musicPlayer.handlerNextSong("next")})},playPrevSong:function(){musicPlayer.controls.prev.on("click",function(e){e.preventDefault();musicPlayer.handlerNextSong("prev")})},handlerNextSong:function(type){if(shuffle){currentSong=getRandomSong();resetAudio();return false}for(var i=
0;i<arraySongs.length;i++)if(currentSong===arraySongs[i])if(type==="next")if(arraySongs[i+1]===undefined)if(settings.loop){currentSong=arraySongs[0];resetAudio();break}else return;else{currentSong=arraySongs[i+1];resetAudio();break}else if(type==="prev"){if(arraySongs[i-1]===undefined)return;currentSong=arraySongs[i-1];resetAudio();break}},shuffleSongs:function(){musicPlayer.controls.shuffle.on("click",function(e){var that=$(this);if(!that.hasClass("active-shuffle")){that.addClass("active-shuffle");
shuffle=true}else{that.removeClass("active-shuffle");shuffle=false}})},init:function(){this.playPause();this.setNameSongPlayer();this.setVolumenSlider();this.muteVolumen();this.playNextSong();this.playPrevSong();this.shuffleSongs();setMute()}};musicPlayer.init()}})(jQuery);
